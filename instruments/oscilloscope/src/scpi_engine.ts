...
import { CursorType, GridType, OscilloscopeSimulation } from "./oscilloscope.ts";
  ...
  private handleChannelCommand(
    ...
    if (sub === "PAVA?") {
      ...
      switch (param) {
        ...
        case "DUTY":
          return `${ch.name}:PAVA DUTY,${(measurements.dutyCycle * 100).toFixed(2)}%`;
        case "ALL":
    ...
    if (sub === "WF?" || sub === "WAVEFORM?") {
      ...
      const sampleList = Array.from(chData.voltage).map((v) => v.toFixed(4))
        .join(",");
      return `${ch.name}:WF DAT2,#9${sampleList.length.toString().padStart(9, "0")}${sampleList}`;
    }
  ...
  private handleWaveformCommand(header: string, args: string): string | null {
    ...
    if (header === "WAV:PRE?" || header === "WAVEFORM:PREAMBLE?") {
      return JSON.stringify({
        ...
        yIncrement: (channelId === 1 ? this.scope.ch1.voltDiv : this.scope.ch2.voltDiv) /
          25,
        yOrigin: channelId === 1 ? this.scope.ch1.offset : this.scope.ch2.offset,
      });
    }
