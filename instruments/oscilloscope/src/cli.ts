...
export class OscilloscopeCli {
  ...
  renderAsciiDisplay(width = 70, height = 18): string {
    ...
    lines.push(
      `${dim}│${reset} ${yellow}CH1: ${frame.ch1.voltDiv}V/div (${
        ...
        `${cyan}CH2: ${frame.ch2.voltDiv}V/div (${frame.ch2.enabled ? "ON" : "OFF"})${reset}  ` +
        `${green}TB: ${(frame.timebase.timeDiv * 1000).toFixed(2)}ms/div${reset}  ` +
    ...
    for (let r = 0; r < gridH; r++) {
      const rowStr = grid[r].map((ch) => ch.includes("\x1b") ? ch : `${dim}${ch}${reset}`).join("");
    ...
    lines.push(
      `${yellow}CH1 Vpp:${m1.vpp.toFixed(2)}V Freq:${m1.frequency.toFixed(1)}Hz${reset} | ` +
        `${cyan}CH2 Vpp:${m2.vpp.toFixed(2)}V Freq:${m2.frequency.toFixed(1)}Hz${reset}`,
    );
    lines.push(`${dim}└${"─".repeat(width - 2)}┘${reset}`);
  ...
  renderMeasurements(): string {
    ...
    return [
      ...
      "----------------+------------------------+--------------------",
      `Vpp (Peak-Peak) | ${m1.vpp.toFixed(3)} V                | ${m2.vpp.toFixed(3)} V`,
      `Vmax (Maximum)  | ${m1.vmax.toFixed(3)} V                | ${m2.vmax.toFixed(3)} V`,
      `Vmin (Minimum)  | ${m1.vmin.toFixed(3)} V                | ${m2.vmin.toFixed(3)} V`,
      `Vrms (RMS)      | ${m1.vrms.toFixed(3)} V                | ${m2.vrms.toFixed(3)} V`,
      `Vavg (Mean)     | ${m1.vavg.toFixed(3)} V                | ${m2.vavg.toFixed(3)} V`,
      `Frequency       | ${m1.frequency.toFixed(2)} Hz             | ${m2.frequency.toFixed(2)} Hz`,
      `Period          | ${(m1.period * 1000).toFixed(3)} ms            | ${
        (m2.period * 1000).toFixed(3)
      } ms`,
      `Duty Cycle      | ${(m1.dutyCycle * 100).toFixed(1)} %                 | ${
  ...
  async runInteractive(): Promise<void> {
    ...
    while (true) {
      ...
      if (input.toLowerCase() === "status") {
        console.log(
          `STB: ${this.scpi.stb} (0b${this.scpi.stb.toString(2).padStart(8, "0")})`,
        );
        console.log(
          `ESR: ${this.scpi.esr} (0b${this.scpi.esr.toString(2).padStart(8, "0")})`,
        );
