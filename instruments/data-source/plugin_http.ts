...
export interface Instrument {
  ...
  command(command: string): string | null | Promise<string | null>;
    ...
    try {
      ...
      switch (path) {
        ...
        case "/configure": {
          ...
          await instrument.configure(config);
          return json(instrument.state());
        }
        case "/command": {
          ...
          const resp = await instrument.command(body.command);
      ...
      if (path.startsWith("/api/")) {
        return await instrument.legacy?.(req, path) ??
