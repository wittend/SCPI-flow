...
export class MultimeterSimulation {
   ...
   */
  private computeRawValue(fn: MeasurementFunction): { val: number; isOverload: boolean } {
    let baseVal = 0;
    const noise = (Math.random() - 0.5) * 2 * this.input.noiseLevel;

    switch (fn) {
      case "VOLT:DC": {
        baseVal = this.input.dcVoltage + noise * 0.005 * Math.abs(this.input.dcVoltage || 1);
        break;
      }
      case "VOLT:AC": {
      ...
      case "CURR:DC": {
        baseVal = this.input.dcCurrent + noise * 0.005 * Math.abs(this.input.dcCurrent || 0.01);
        break;
      }
      case "CURR:AC": {
      ...
      case "FRES": {
        ...
        baseVal = this.input.resistance + noise * 0.0005 * this.input.resistance;
        break;
      }
      case "CAP": {
        baseVal = this.input.capacitance + noise * 0.005 * this.input.capacitance;
        break;
      }
      case "CONT": {
  ...
  public formatValue(
    ...
    switch (fn) {
      case "VOLT:DC": {
        ...
        if (rng <= 0.6) {
          return { formatted: `${sign}${(absVal * 1000).toFixed(2)} mVDC`, unit: "mV" };
        } else if (rng <= 6.0) {
      ...
      case "VOLT:AC": {
        const rng = this.range;
        if (rng <= 0.6) {
          return { formatted: `${(absVal * 1000).toFixed(2)} mVAC`, unit: "mV" };
        } else if (rng <= 6.0) {
      ...
      case "CURR:DC": {
        const rng = this.range;
        if (rng <= 0.0006) {
          return { formatted: `${sign}${(absVal * 1e6).toFixed(2)} uADC`, unit: "uA" };
        } else if (rng <= 0.006) {
          return { formatted: `${sign}${(absVal * 1000).toFixed(4)} mADC`, unit: "mA" };
        } else if (rng <= 0.06) {
          return { formatted: `${sign}${(absVal * 1000).toFixed(3)} mADC`, unit: "mA" };
        } else if (rng <= 0.6) {
          return { formatted: `${sign}${(absVal * 1000).toFixed(2)} mADC`, unit: "mA" };
        } else if (rng <= 6.0) {
      ...
      case "CURR:AC": {
        const rng = this.range;
        if (rng <= 0.06) {
          return { formatted: `${(absVal * 1000).toFixed(3)} mAAC`, unit: "mA" };
        } else if (rng <= 0.6) {
          return { formatted: `${(absVal * 1000).toFixed(2)} mAAC`, unit: "mA" };
        } else if (rng <= 6.0) {
      ...
      case "FRES": {
        ...
        if (absVal < 1000) {
          ...
          return { formatted: `${(absVal / 1e3).toFixed(4)} k${suffix}`, unit: "kΩ" };
        } else {
          return { formatted: `${(absVal / 1e6).toFixed(4)} M${suffix}`, unit: "MΩ" };
        }
      }
      case "CAP": {
        if (absVal < 1e-6) {
      ...
      case "FREQ": {
        if (absVal < 1000) {
          ...
          return { formatted: `${(absVal / 1000).toFixed(4)} kHz`, unit: "kHz" };
        } else {
  ...
  public takeReading(): ReadingResult {
    ...
    // Apply dB / dBm Math
    if (this.mathFunction === "DB" && !isOverload && this.function.startsWith("VOLT")) {
      const ref = this.dbReference || 1.0;
      finalVal = 20 * Math.log10(Math.max(1e-12, Math.abs(rawVal)) / ref);
    } else if (this.mathFunction === "DBM" && !isOverload && this.function.startsWith("VOLT")) {
    ...
    const formattedResult = this.formatValue(finalVal, this.function, isOverload);
    ...
    if (this.dualEnabled) {
      ...
      const secFmt = this.formatValue(sec.val, this.secondaryFunction, sec.isOverload);
  ...
  public addSampleToStatistics(val: number): void {
    ...
    const variance =
      this._statSamples.reduce((acc, v) => acc + Math.pow(v - this.stats.average, 2), 0) / (n - 1);
  ...
  public addSampleToHistogram(val: number): void {
    ...
    const binIdx = Math.floor(((val - this.histogram.lower) / span) * this.histogram.binCount);
    const clampedIdx = Math.max(0, Math.min(this.histogram.binCount - 1, binIdx));
    this.histogram.bins[clampedIdx] = (this.histogram.bins[clampedIdx] || 0) + 1;
  }
}
