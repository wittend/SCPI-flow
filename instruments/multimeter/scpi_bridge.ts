 ...
 * Digital Multimeter simulation engine and optional Python Bridge (pyVisa).
 */
...
export class ScpiBridge {
  ...
  constructor(meter?: MultimeterSimulation) {
    this.engine = new ScpiEngine(meter);
  }
  ...
  async sendCommand(payload: {
    ...
    const resource = payload.resource || "SIM::MULTIMETER::SDM3045X";
    ...
    if (
      ...
      if (payload.command === "list_resources") {
        return {
          status: "success",
          resources: [
            "SIM::MULTIMETER::SDM3045X",
            "SIM::INPUT::TERMINAL_HI_LO",
            "SIM::INPUT::TERMINAL_SENSE",
            "SIM::INPUT::TERMINAL_CURRENT",
          ],
        };
      }
